...
CREATE DEFINER=`root`@`%` PROCEDURE `projects`.`create_part_for_project`(
...
CREATE DEFINER=`root`@`%` PROCEDURE `projects`.`create_task_for_project`(
    in project_id integer,
    in name varchar(128), 
    in description text,
    in status int,
